...
def upload_asset(file_path, base_url, api_key):
    ...
    device_asset_id = (
        f"{os.path.basename(file_path)}-{stats.st_size}-{int(stats.st_mtime)}"
    )
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            # CASE 2: 200 OK (Soft Duplicate - Immich matched the checksum/ID)
                ...
                logging.warning(
                    f"File exists (Deduplicated): {os.path.basename(file_path)}"
                )
                return response.json().get("id")
...
def main():
    ...
    if not target_album_id:
        logging.error(
            f"Album '{ALBUM_NAME}' not found on server! Please create it in Immich web UI."
        )
        return
